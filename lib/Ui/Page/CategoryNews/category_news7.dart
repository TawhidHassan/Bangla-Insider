...
class CategoryNews7 extends StatefulWidget {
  int? menuId;
  int? lan;
  String? category;

  FooterContent?footerContent;
  CategoryNews7({this.menuId, this.lan,this.category,this.footerContent});

  @override
  _CategoryNews7State createState() => _CategoryNews7State();
}

class _CategoryNews7State extends State<CategoryNews7> {
  @override
  void initState() {
    // TODO: implement initState
    print("running cattt");
    BlocProvider.of<CategoryNews7Cubit>(context).categoryNewsData1(widget.menuId,widget.lan);
    super.initState();
  }
  @override
  Widget build(BuildContext context) {
    return BlocBuilder<CategoryNews7Cubit, CategoryNews7State>(
      builder: (context, state) {
        if(state is !HomeCatnewsData7){
          return HomeCategoryShimer();
        }
        final data=(state as HomeCatnewsData7).categoryNewsResponse;
