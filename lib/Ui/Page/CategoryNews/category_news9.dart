...
class CategoryNews9 extends StatefulWidget {
  int? menuId;
  int? lan;
  String? category;

  FooterContent?footerContent;
  CategoryNews9({this.menuId, this.lan,this.category,this.footerContent});

  @override
  _CategoryNews9State createState() => _CategoryNews9State();
}

class _CategoryNews9State extends State<CategoryNews9> {
  @override
  void initState() {
    // TODO: implement initState
    print("running cattt");
    BlocProvider.of<CategoryNews9Cubit>(context).categoryNewsData1(widget.menuId,widget.lan);
    super.initState();
  }
  @override
  Widget build(BuildContext context) {
    return BlocBuilder<CategoryNews9Cubit, CategoryNews9State>(
      builder: (context, state) {
        if(state is !HomeCatnewsData9){
          return HomeCategoryShimer();
        }
        final data=(state as HomeCatnewsData9).categoryNewsResponse;
