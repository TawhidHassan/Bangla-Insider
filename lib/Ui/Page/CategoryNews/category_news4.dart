...
class CategoryNews4 extends StatefulWidget {
  int? menuId;
  int? lan;
  String? category;

  FooterContent?footerContent;
  CategoryNews4({this.menuId, this.lan,this.category,this.footerContent});
  @override
  _CategoryNews4State createState() => _CategoryNews4State();
}

class _CategoryNews4State extends State<CategoryNews4> {
  @override
  void initState() {
    // TODO: implement initState
    print("running cattt");
    BlocProvider.of<CategoryNews4Cubit>(context).categoryNewsData1(widget.menuId,widget.lan);
    super.initState();
  }
  @override
  Widget build(BuildContext context) {
    return BlocBuilder<CategoryNews4Cubit, CategoryNews4State>(
      builder: (context, state) {
        if(state is !HomeCatnewsData4){
          return HomeCategoryShimer();
        }
        final data=(state as HomeCatnewsData4).categoryNewsResponse;
              ...
              Container(
                child: Column(
                  children: data.data!.content!.map((content) =>
                      SportsNews(cover:content,footerContent: widget.footerContent,),
                  ).toList(),
                ),
              ),
