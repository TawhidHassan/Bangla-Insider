import 'package:banglainsider/Bloc/CategoryNews/category_news21_cubit.dart';
...
import 'package:banglainsider/Ui/Widgets/HighlightNews/high_light_news_cicle_iamge.dart';
import 'package:banglainsider/Ui/Widgets/Simmer/home_category_simmer.dart';
import 'package:flutter/material.dart';
import 'package:flutter_bloc/flutter_bloc.dart';

class CategoryNews21 extends StatefulWidget {
  int? menuId;
  int? lan;
  String? category;

  FooterContent?footerContent;
  CategoryNews21({this.menuId, this.lan,this.category,this.footerContent});

  @override
  _CategoryNews21State createState() => _CategoryNews21State();
}

class _CategoryNews21State extends State<CategoryNews21> {
  @override
  void initState() {
    // TODO: implement initState
    print("running cattt");
    BlocProvider.of<CategoryNews21Cubit>(context).categoryNewsData1(widget.menuId,widget.lan);
    super.initState();
  }
  @override
  Widget build(BuildContext context) {
    return BlocBuilder<CategoryNews21Cubit, CategoryNews21State>(
      builder: (context, state) {
        if(state is !HomeCatnewsData21){
          return HomeCategoryShimer();
        }
        final data=(state as HomeCatnewsData21).categoryNewsResponse;
              ...
              Container(
                child: Column(
                  children: data.data!.content!.map((content) =>
                      HeighLightNewsCircleImage(cover:content,footerContent: widget.footerContent,),
                  ).toList(),
                ),
              ),
